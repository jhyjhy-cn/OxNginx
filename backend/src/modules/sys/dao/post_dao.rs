...
pub async fn list_posts_paged(
    ...
    if let Some(ref pattern) = like {
        ...
        let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sys_posts")
            .fetch_one(pool)
            .await?;
        let rows = sqlx::query_as::<_, Post>(
...
pub async fn insert_post_returning_id(
    pool: &SqlitePool,
    code: &str,
    name: &str,
    sort: i32,
...
pub async fn delete_post(pool: &SqlitePool, id: i64) -> sqlx::Result<u64> {
    let r = sqlx::query("DELETE FROM sys_posts WHERE id=?")
        .bind(id)
        .execute(pool)
        .await?;
    Ok(r.rows_affected())
