...
use crate::modules::backup::service::backup_service;
use crate::AppState;
...
pub async fn create_backup(
    ...
    // 获取站点当前配置
    let site = match crate::modules::site::service::site_service::get_site(&state, site_id).await {
        Ok(Some(s)) => s,
        Ok(None) => return Json(json!(ApiResponse::<()>::error("站点不存在"))),
        Err(e) => return Json(json!(ApiResponse::<()>::error(format!("获取站点失败: {}", e)))),
    };

    let config = crate::modules::common::nginx::generate_site_config(&site);
