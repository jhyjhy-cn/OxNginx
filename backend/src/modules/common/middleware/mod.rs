...
pub async fn require_admin(
    ...
    let username = if let Some(info) = request.extensions().get::<TokenInfo>().cloned() {
        ...
        match crate::modules::auth::service::token_service::verify_token(state.db.pool(), token).await {
            Ok(Some(u)) => u,
            _ => return Err(StatusCode::UNAUTHORIZED),
        }
    };
    if username == "admin" { return Ok(next.run(request).await); }
    match crate::modules::sys::service::user_service::user_is_super_admin(&state.db.pool(), &username).await {
