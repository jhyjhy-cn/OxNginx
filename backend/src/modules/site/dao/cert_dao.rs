...
pub async fn list_all_certs(pool: &SqlitePool) -> sqlx::Result<Vec<Certificate>> {
    sqlx::query_as::<_, Certificate>("SELECT * FROM site_certificates ORDER BY created_at DESC")
        .fetch_all(pool)
        .await
}

pub async fn find_cert_by_id(pool: &SqlitePool, id: i64) -> sqlx::Result<Option<Certificate>> {
    sqlx::query_as::<_, Certificate>("SELECT * FROM site_certificates WHERE id = ?")
        .bind(id)
        .fetch_optional(pool)
        .await
}
...
pub async fn find_cert_by_domain(pool: &SqlitePool, domain: &str) -> sqlx::Result<Option<Certificate>> {
    sqlx::query_as::<_, Certificate>("SELECT * FROM site_certificates WHERE domain = ?")
        .bind(domain)
        .fetch_optional(pool)
        .await
}
...
pub async fn update_cert_paths(
    ...
    sqlx::query("UPDATE site_certificates SET cert_path = ?, key_path = ? WHERE id = ?")
        .bind(cert_path)
        .bind(key_path)
        .bind(id)
        .execute(pool)
...
pub async fn insert_cert_returning(
    ...
    sqlx::query_as::<_, Certificate>(
        r#"
        INSERT INTO site_certificates (domain, issuer, cert_path, key_path, auto_renew)
