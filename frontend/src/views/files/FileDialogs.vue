  ...
  <OnDialog v-model="propDialog.visible" title="属性" width="480px">
    <el-descriptions :column="1" border size="small">
      ...
      <el-descriptions-item label="类型">
        {{ propDialog.item?.is_dir ? '文件夹' : '文件' }}{{ propDialog.item?.extension ? ' (.' + propDialog.item.extension + ')' : '' }}
      ...
      <el-descriptions-item label="大小">
        <template v-if="propDialog.item?.is_dir">
          ...
          <el-button
            ...
            type="primary"
            size="small"
            :loading="propDialog.item?._calcLoading"
            @click="propDialog.item && $emit('calc-size', propDialog.item)"
          >
            计算
          </el-button>
        </template>
      ...
      <el-descriptions-item label="权限/所有者">
        {{ propDialog.item?.permissions || '-' }}{{ propDialog.item?.owner ? ' / ' + propDialog.item.owner : '' }}
      </el-descriptions-item>
